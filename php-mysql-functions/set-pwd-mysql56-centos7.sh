...
# comment line 61 to change password even if one exists and set "-p='existing password'" on line 65 *********
...
echo "Database root password set successfully"

config=/root/.my.cnf
# Make the auto login mysql file
echo "[client]" > $config
echo "# for local server use localhost" >> $config
echo "host=localhost" >> $config
echo "#host=127.0.0.1" >> $config
echo "user=root" >> $config
echo "password='$db_root_password'" >> $config
echo "" >> $config
echo "[mysql]" >> $config
echo "pager=/usr/bin/less" >> $config
echo "" >> $config

echo "root user auto login file $config prepared"
